...
;(function(window) {
	...
	Point.prototype.mbhide = function() {
		lunar.addClass(this.el , 'point--mbhide');
	}
	Point.prototype.removembhide = function() {
		lunar.removeClass(this.el, 'point--mbhide');
	}
	/**
	 * 
	 */
		...
		[].slice.call(this.pointsWrapper.querySelectorAll('.point')).forEach(function(point, pos) {
			var p = new Point(point, self.bgElems[pos], self.el, {
				...
				onClick : function() {
					...
					self._pointsAction('removembhide');
					// Hide tooltip.
					self.tooltips[pos].classList.remove('point-tooltip--current');
